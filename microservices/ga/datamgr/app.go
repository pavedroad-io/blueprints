...
func (a *{{.NameExported}}App) initializeRoutes() {
  ...
  a.Router.HandleFunc(uri, a.options{{.NameExported}}).Methods("OPTOINS")
...
func respondOptions(w http.ResponseWriter, code int, payload interface{}) {
	...
	w.Header().Set("Access-Control-Allow-Headers", "Access-Control-Allow-Origin, DNT,User-Agent,X-Requested-With,If-Modified-Si  nce,Cache-Control,Content-Type,Range")
